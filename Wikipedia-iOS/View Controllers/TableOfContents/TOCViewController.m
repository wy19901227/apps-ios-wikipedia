...
- (void)viewDidLoad
{
    ...
    // Get cell for "Read in another language" button beneath table of contents cells.
    [self getLanguageCell];
...
-(void)navigateToSelection:(UITapGestureRecognizer *)sender
{
    ...
    }else if ([subview isMemberOfClass:[TOCNonSectionCellView class]]) {
        switch (subview.tag) {
            case TOC_TAG_OTHER_LANGUAGES:
            
                NSLog(@"label tapped %@ %d", ((TOCNonSectionCellView *)subview).text, subview.tag);
...
#pragma mark Non section cells

-(void)getLanguageCell
{
    // Add a "Read in another language" button, etc, beneath the section cells.
    if (self.languageCount.integerValue == 1) return;
    TOCNonSectionCellView *cell = [[TOCNonSectionCellView alloc] init];
    cell.translatesAutoresizingMaskIntoConstraints = NO;
    cell.text = @"Read in another language";
    cell.tag = TOC_TAG_OTHER_LANGUAGES;
    [self.sectionCells addObject:cell];
}
