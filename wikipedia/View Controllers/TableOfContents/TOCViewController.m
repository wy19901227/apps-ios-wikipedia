...
@property (strong, nonatomic) NSMutableArray *tocSectionData;
...
- (instancetype)initWithCoder:(NSCoder *)coder
{
    ...
    if (self) {
        self.tocSectionData = @[].mutableCopy;
    }
    return self;
}
...
-(void)refreshForCurrentArticle
{
    ...
    // iOS 6 needs constraints to be removed before removeFromSuperview is called.
    for (UIView *v in [self.scrollContainer.subviews copy]) {
        [v removeConstraintsOfViewFromView:v.superview];
    }
    [[self.scrollContainer.subviews copy] makeObjectsPerformSelector:@selector(removeFromSuperview)];
...
-(void)constrainSectionCells
{
    ...
    for (UIView *cell in [self.sectionCells copy]) {
        [cell removeConstraintsOfViewFromView:self.scrollContainer];
    }
...
-(void)setTocSectionDataForSections:(NSSet *)sections
{
    ...
    [self.tocSectionData removeAllObjects];
    ...
    for (Section *section in [sortedSection copy]) {
        ...
        [self.tocSectionData addObject:sectionDict];
