...
-(void)downloadAssetsFilesIfNecessary
{
    ...
    [[QueuesSingleton sharedInstance].assetsRetrievalManager.operationQueue cancelAllOperations];

    // "Set the responseSerializer to AFHTTPResponseSerializer, so that it will no longer
    // try to parse the JSON (and will automatically accept text/html)"
    // From: http://stackoverflow.com/a/21621530
    [QueuesSingleton sharedInstance].assetsRetrievalManager.responseSerializer = [AFHTTPResponseSerializer serializer];

    void (^fetch)(AssetsFileEnum) = ^void(AssetsFileEnum type) {
        AssetsFileFetcher *fetcher = [[AssetsFileFetcher alloc] init];
        [fetcher fetchAssetsFile: type
                          maxAge: maxAge
                     withManager: [QueuesSingleton sharedInstance].assetsRetrievalManager];
...
- (void)didReceiveMemoryWarning
{
    ...
    */
    ...
    //NSLog(@"articleRetrievalManager.operationCount = %lu", (unsigned long)[QueuesSingleton sharedInstance].articleRetrievalManager.operationQueue.operationCount);
}
...
- (void)fetchFinishedForObject: (Article *)article
                        sender: (id)sender
                         ...
                         error: (NSError *)error
{
    ...
    switch (type) {
        case ARTICLE_FETCH_TYPE_SECTIONS_LEAD:
            
            switch (result) {
                case FETCH_RESULT_SUCCEEDED:
                {
        ...
        case ARTICLE_FETCH_TYPE_SECTIONS_NONLEAD:
                    ...
                    discoveryMethod: (NSString *)discoveryMethod
{
    ...
    [articleDataContext_.mainContext performBlockAndWait:^(){
        ...
        // Means "fetchFinishedForObject:sender:result:type:error:" above will
        // be notified when articleFetcher's "download" method has actually
        // retrieved some data.
        ArticleFetcher *articleFetcher = [[ArticleFetcher alloc] init];
        articleFetcher.articleDelegate = article;
        articleFetcher.fetchCompletionDelegate = self;
        // Kick off the download.
        [articleFetcher fetchWithManager:[QueuesSingleton sharedInstance].articleRetrievalManager];
