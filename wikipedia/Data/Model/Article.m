...
// Imports to support the "download" method.
...
-(void)downloadWithManager:(AFHTTPRequestOperationManager *)manager
{
    if(!self.domain){
        NSLog(@"NO DOMAIN");
        return;
    ...
    void (^getNonLeadSections)() = ^void() {
        ...
        NSDictionary *params = [self getParamsForTitle:self.title leadSectionOnly:NO];
        ...
        [manager GET:url.absoluteString parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            NSDictionary *nonLeadSectionsResults = [self prepareResultsFromResponse:responseObject forTitle:self.title];
            
            [self applyResultsForNonLeadSections:nonLeadSectionsResults];

            [self downloadFinishedWithResult: ARTICLE_DOWNLOAD_RESULT_SUCCESS
            ...
            if([error code] == NSURLErrorCancelled) {
                [self downloadFinishedWithResult: ARTICLE_DOWNLOAD_RESULT_CANCELLED
                                            type: ARTICLE_DOWNLOAD_TYPE_SECTIONS_NONLEAD
                                           error: error];
            }else{
                [self downloadFinishedWithResult: ARTICLE_DOWNLOAD_RESULT_FAILED
                                            type: ARTICLE_DOWNLOAD_TYPE_SECTIONS_NONLEAD
                                           error: error];
            }
        }];
    ...
    NSDictionary *params = [self getParamsForTitle:self.title leadSectionOnly:YES];
    ...
    [manager GET:url.absoluteString parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        NSDictionary *leadSectionResults = [self prepareResultsFromResponse:responseObject forTitle:self.title];
...
-(void)applyResultsForLeadSection:(NSDictionary *)results
{
    [self.managedObjectContext performBlockAndWait:^(){
        ...
        self.languagecount = results[@"languagecount"];
        self.lastmodified = results[@"lastmodified"];
        self.lastmodifiedby = results[@"lastmodifiedby"];
        self.articleId = results[@"articleId"];
        self.editable = results[@"editable"];
        self.protectionStatus = results[@"protectionStatus"];
        ...
        self.redirected = results[@"redirected"];
...
- (void)downloadFinishedWithResult: (DownloadResult)result
